...
- タスク開始時に必ず`apply_patch`で`[ ]`→`[x]`に更新
- タスク完了時に必ず`apply_patch`で完了を記録
- **tasklist.mdの全タスクが完了するまで作業を継続する**
- NEVER: tasklist.mdを更新せずに次のタスクに進まない

**NEVER（禁止）**:
- tasklist.mdを見ずに実装を進める
- 作業計画の進捗更新だけで進捗管理する（作業計画の進捗情報は補助、tasklist.mdが正式）
...
```
対象ファイル `.steering/[日付]-[機能名]/tasklist.md` を読む
```
...
tasklist.mdの内容に基づいて作業計画の進捗を更新し、タスクリストを管理:
- これはCodex内部の補助的なメモ
...
```
`apply_patch`を使って、tasklist.mdの該当行を`[ ]`→`[x]`に更新
...
**重要**: `apply_patch`を実行した直後に、更新が成功したことを確認する。

**3-3. 作業計画でもステータス更新**
```
作業計画の進捗を更新し、該当タスクを"in_progress"に変更
```
...
```
実装完了後、必ず`apply_patch`でtasklist.mdを更新して完了を記録
...
**3-6. 作業計画でもステータス更新**
```
作業計画の進捗を更新し、該当タスクを"completed"に変更
```
...
2. **振り返りセクションに記録**
   ```
   `apply_patch`でtasklist.mdの「実装後の振り返り」セクションを更新:
   - 実装完了日
   - 計画と実績の差分
   - 学んだこと
   - 次回への改善提案
...
- [ ] 進捗がドキュメントに反映されているか？（対象ファイルを読んで確認）
...
1. **即座に更新を実行**
   ```
   対象ファイル `.steering/[日付]-[機能名]/tasklist.md` を読む
   完了したタスクを特定し、すべて`apply_patch`で`[x]`に更新
   ```
...
1. **tasklist.mdに注釈を追加**
   ```
   `apply_patch`で該当タスクに注釈を追加:
   「- [x] タスク名（実装方法を変更: 理由）」
   ```

2. **必要に応じて新しいタスクを追加**
   ```
   `apply_patch`で新しいタスクを追加
   ```
...
- [ ] tasklist.mdを読み込んだか？
- [ ] 次のタスクを特定したか？
- [ ] タスク開始時に`apply_patch`で更新したか？

実装後に必ず確認:

- [ ] タスク完了時に`apply_patch`で更新したか？
