...
| 巻きネジ障害機 | Winder | ステージ内を徘徊する時計仕掛けの障害機。地上を歩き壁・段差端で反転する。`'E'` タイルで配置 |
| 時計トンボ | Clock Dragonfly / Flyer | 空中を水平巡回しつつ上下にサイン揺れする飛行敵。重力無効で地形を無視する。`'F'` タイルで空中配置（Stage 02 から登場） |
| チクタク爆弾 | Ticking Bomb | プレイヤーを検知すると追尾し、至近で導火（点滅）→自爆して範囲ダメージを与える地上敵。踏めば導火前に解除できる。`'B'` タイルで地上配置（Stage 03 から登場） |
| 自爆 | Self-destruct / Explode | チクタク爆弾が導火完了または接触時に爆発し、爆風半径（`BOMB_BLAST_RADIUS_PX`）内のプレイヤーへ被弾を与える挙動 |
| 歯車片 | Gear Bit | プレイヤーが触れると取得できる収集物。`'C'` タイルで配置 |
| 予備ゼンマイ | Heal Item | プレイヤーが触れるとライフを 1 回復する救済アイテム（上限 `INITIAL_LIVES`=3）。`'H'` タイルで通常各ステージにちょうど 1 個配置。真鍮歯車枠＋ルビーのハートの意匠 |
