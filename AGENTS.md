...
| skill | 発火条件 |
|-------|---------|
| `prd-writing` | `docs/product-requirements.md` の新規作成・大幅改訂時 |
| `functional-design` | `docs/functional-design.md` の新規作成・大幅改訂時 |
| `architecture-design` | `docs/architecture.md` の新規作成・大幅改訂時 |
| `repository-structure` | `docs/repository-structure.md` の新規作成・大幅改訂時 |
| `development-guidelines` | `docs/development-guidelines.md` の新規作成・大幅改訂時 |
| `glossary-creation` | `docs/glossary.md` の新規作成・大幅改訂時 |
| `steering` | `.steering/[YYYYMMDD]-[開発タイトル]/` 配下のドキュメント作成・更新時 |
| `setup-project` | 初回セットアップとして6つの永続ドキュメントを作成する時 |
| `add-feature` | ユーザーが `$add-feature` を明示指定した時のみ |
| `review-docs` | 永続ドキュメントまたはステアリングファイルをレビューする時 |

定義は `.agents/skills/<skill-name>/SKILL.md` に格納。

### Codex カスタムエージェント

Claude 側のドキュメントレビュー・実装検証担当は、Codex では以下の project custom agent として定義する。

| agent | 定義ファイル | 用途 |
|-------|-------------|------|
| `doc-reviewer` | `.codex/agents/doc-reviewer.toml` | `docs/` / `.steering/` のレビュー |
| `implementation-validator` | `.codex/agents/implementation-validator.toml` | 実装完了後の要件・テスト・セキュリティ整合性確認 |
